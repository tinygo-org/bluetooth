// Example that demonstrates BLE passkey display pairing in the peripheral
// role, currently supported on Nordic SoftDevices only.
//
// Connect with a central and try to read a characteristic gated on an
// authenticated (MITM protected) link: this triggers pairing, and the
// passkey to enter on the central is printed on the serial console.
//
...
// changing the PairingParams:
//
//   - Passkey entry: IOCapabilities: bluetooth.IOCapsKeyboardOnly and set
//     PasskeyEntryHandler, which must call device.EnterPasskey with the
//     passkey shown by the central.
//   - Legacy pairing: LESC: false.
package main
...
func main() {
	...
	must("enable pairing", adapter.EnablePairing(bluetooth.PairingParams{
		IOCapabilities: bluetooth.IOCapsDisplayOnly,
		LESC:           true,
		MITM:           true,
		PasskeyDisplayHandler: func(device bluetooth.Device, passkey string) {
			println("*** enter this passkey on the central:", passkey)
		},
		...
		Characteristics: []bluetooth.CharacteristicConfig{
			{
				...
				ReadSecurity: bluetooth.SecurityAuthenticated,
			},
		},
	}))
	...
	must("add HID service", adapter.AddService(&bluetooth.Service{
		...
		Characteristics: []bluetooth.CharacteristicConfig{
			...
			},
			{
				...
				WriteSecurity: bluetooth.SecurityAuthenticated,
			},
		},
	}))
